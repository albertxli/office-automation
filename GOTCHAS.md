...
### #49 Unpaired Special Shapes Were Silent (Rust-specific)
Pairing is **OLE-driven**: for each OLE object, the first `delt*_` / `ntbl_` / `htmp_` / `trns_` shape on the same slide whose name contains the OLE name as a whole token is taken (`inventory.rs`). A misnamed OLE (`globalnet_g` next to `delt2_globalnet_f`, v0.8 deck slide 15) therefore leaves the delta untouched — no `_pos/_neg/_none` suffix, no error — and `oa check` never saw it either, because `check_deltas` also walks OLE objects and skipped suffix-less deltas.

**Fix (v0.3.8):** `build_inventory` keeps the leftovers — `unpaired_delts`, `unpaired_tables`, `unpaired_oles`, plus `ole_names_by_slide` — and `SlideInventory::unpaired_special_shapes()` pairs each one with the closest OLE name on its slide (`matcher::special_token` strips `delt2_`/`ntbl_` and the sign suffix; `matcher::closest_name` = Levenshtein ≤ 2, ties by shared prefix). Reported in three places: `oa update` warns `Slide 15 │ delt2_globalnet_f · no OLE object matches on this slide (closest OLE name: globalnet_a) — not updated`; `oa check` has a **Pairs** row (unpaired special shapes fail the check; a paired delta with no suffix is now a delta mismatch "never updated"); `oa info` has a Pairing section. **OLE objects without a table/delta are legitimate** (standalone linked pictures) — counted and listed under `-v`, never a failure. Note the hint is a nearest-name guess: with `globalnet_a…e` and `_g` all one edit away from `_f`, it names the first tie, so read it as "one of these" rather than "this one".
